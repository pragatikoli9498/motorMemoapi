...
namespace MotorMemo.Controllers.Reports
{
    ...
    public class TrialBalanceReportController : ControllerBase
    {
        ...
        public readonly TrialBalance1Proc _proc1;
        public readonly CommanProc _comman;
        private respayload rtn = new respayload();
        public TrialBalanceReportController(IMemoryCache cache, IWebHostEnvironment environment, TrialBalanceProc proc,TrialBalance1Proc proc1, CommanProc comman)
        {
            _cache = cache;
        ...
        public async Task<IActionResult> Report([FromBody] Dictionary<string, object> jsonArray)
        {
            ...
            if (_reportCacheId != null)
            {
                ...
                string[] DbParamNames = new string[] { "firm_id", "div_id", "sdt", "edt", "reportOption" };
                var DbParamsWithValue = repoService.getdbParams(reportParams, DbParamNames);

                string? reportOption = reportParams.Where(w => w.key == "reportOption").Select(s => s.value).SingleOrDefault();
                ...
                string[] RpParamNames = ((Convert.ToInt16(reportOption) == 1) ? new string[] { "edt" } : new string[] { "sdt", "edt" });
                var RpParamsWithValue = repoService.getReportParams(reportParams, RpParamNames);

                string basePath = Environment.ContentRootPath;

                string RdlPath = basePath + ((Convert.ToInt16(reportOption) == 1) ? @"\\Models\\Rdlc\\Trial_Balance.rdl" : @"\\Models\\Rdlc\\Trial_Balance1.rdl");
                ...
                try
                {
                    ...
                    if (ReportData == null)
                    {
                        ...
                        object dataset1 = ((Convert.ToInt64(reportOption) == 1) ?
                                            ...
                                            DbParamsWithValue["div_id"].ToString(),
                                             ...
                                             Convert.ToDateTime(DbParamsWithValue["edt"])
                                              ...
                                              await _proc1.Data1(Convert.ToInt16(DbParamsWithValue["firm_id"]),
                                           DbParamsWithValue["div_id"].ToString(),
                                              Convert.ToDateTime(DbParamsWithValue["sdt"]),
                                            Convert.ToDateTime(DbParamsWithValue["edt"])
