...
{
    public class SundryWiseProc
    {
        private readonly MotorMemoDbContext _context;
        public SundryWiseProc(IOptions<ConnectionString> SqlLiteConnstring, IConfiguration configuration, MotorMemoDbContext context)
        ...
        public async Task<object> Data(int firm_id, string div_id,int s_id, DateTime? sdt, DateTime edt)
        {
            ...
            using (var ocomm = new SqliteConnection(_context.Database.GetConnectionString()))
            {
                ...
                var CommandText = @"SELECT MEMO.memo_No, memo.dt,MEMO.from_dstn,MEMO.to_dstn,MEMO.vehicle_no,Sundry.sundry_name,EXP.charges FROM motormemo MEMO
